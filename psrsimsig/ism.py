            ...
            for ii, freq in enumerate(self.freq_Array):
                ...
                if (ii+1) % int(self.Nf//20) ==0:
                    ...
                    try: #Python 2 workaround. Python 2 __future__ does not have 'flush' kwarg.
                        print('\r{0:2.0f} dispersed in {1} seconds.'.format(round((ii + 1)*100/self.Nf) , shift_check-shift_start), end='', flush=True)
                    except: #This is the Python 2 version.
                        print('\r{0:2.0f} dispersed in {1} seconds.'.format(round((ii + 1)*100/self.Nf) , shift_check-shift_start), end='')
                    sys.stdout.flush()
